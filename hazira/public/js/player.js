...
const screens = {
  join: $('s-join'), wait: $('s-wait'), pick: $('s-pick'),
...
function renderWait() {
  ...
  $('wait-note').textContent = state.players.length >= 2
    ? `${state.players.length} מתמודדים בזירה — ממתינים לפתיחה`
    : 'ממתינים למתמודדים נוספים…';
